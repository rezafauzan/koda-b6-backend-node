    ...
    try {
        ...
        const sqlItems = `
            SELECT 
                ...
                v.additional_price AS variant_price,
                s.portion_size AS size_name,
                s.additional_price AS size_price
            FROM cart_items ci
        ...
        const sqlOrder = `
            ...
            RETURNING id, cart_id, total, status, fullname, phone, email, address, delivery, created_at, updated_at;
        `;

        const orderResult = await client.query(sqlOrder, [
            cart_id,
            0,
            1,
            fullname,
        ...
        for (const item of itemsResult.rows) {
            const unit_base_price = Number(item.base_price);
            const variant_price = Number(item.variant_price || 0);
            const portion_price = Number(item.size_price || 0);

            const unit_final_price =
                unit_base_price + variant_price + portion_price;

            const total_price =
                unit_final_price * item.quantity;
